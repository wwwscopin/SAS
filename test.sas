	proc format;
		value gender 1="01 X Male" 2="Female";
...
data test;
	...
	label gender="Gender"
		  ...
		  grade="Grade";
	format gender gender. lungcancer type. grade grade.;
run;

proc report data=test nowd headskip missing
contents="by treatment group"
style(report)={font_weight=bold pretext="Demo 4 - Customized Summary Tables: Gender and Weight\line\By Treatment Group"};
column gender lungcancer age grade value(height);
define gender/ group order=data;
define lungcancer/group 'Lung Cancer ' order=data style(header)={asis=on just=left vjust=middle} style(column)={asis=on cellwidth=5cm just=l};
define grade/across order=data;
define age/format=4.0 'Age' style(column)={cellwidth=3cm just=c};
...
%macro cat_vars (var= , format=, label=);
        ...
        proc freq data= test compress noprint;
	...
	%cat_vars(var=LUNGCANCER, format= type., label= "LUNGCANCER");
	%cat_vars(var=grade, format= type., label= "Grade");

	data demo_table;
        set cat_table;
        by variable notsorted;
		tmp = varlabel(open('test', 'i'), varnum(open('test'), 'lungcancer'));
        output;
	...
	proc sort; by variable row;run;


options nodate nonumber orientation = portrait;
ods rtf file = "test.rtf" style=journal;
    title "Allergen Study Demographic Summary ";
    proc print data = demo_tab noobs label style(header) = [just=center] split = "*";
        id variable;
        by variable notsorted;
        var  row;
    run;
...
%descriptive_stat(data_in= test, data_out= tmp, var= Gender, type= cat, first_var=1);
%descriptive_stat(data_in= test, data_out= tmp, var= lungcancer, type= cat);
%descriptive_stat(data_in= test, data_out= tmp, var= grade, type= cat, last_var=1);
proc print;run;
